...
uuid that fails (not found, or a request error) prints its own `fetch_failed`
object to stderr and the exit code is non-zero, while stdout still carries the
JSON array of whichever records were found — a script should check the exit
code rather than assume a valid JSON array on stdout means every requested
uuid was returned.
